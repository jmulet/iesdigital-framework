 ...
 */
public class JRDayChooser extends JPanel implements ActionListener, KeyListener,
		FocusListener {
	...
	protected MinMaxDateEvaluator minMaxDateEvaluator;
        ...
        protected  boolean shiftLessVisible = false;
        protected  boolean shiftMoreVisible = false;
    private final JButton shiftLess;
    private final JButton shiftMore;
	 ...
	 */
	public JRDayChooser(boolean weekOfYearVisible) {
		setName("JDayChooser");
		setBackground(Color.blue);

                //This is to reduce button size
                LookAndFeel laf = UIManager.getLookAndFeel();
                if (laf != null) {
                    UIDefaults def = laf.getDefaults();
                    def.put("Button.contentMargins", new Insets(5, 5, 5, 5));
                }
                ...
                jLabelTitle.setBackground(new Color(0,0,175));
		...
		for (int y = 0; y < 7; y++) {
			for (int x = 0; x < 7; x++) {
				...
				days[index].setMargin(new Insets(0, 0, 0, 0));
                               // days[index].setFont(new Font("Arial",Font.PLAIN, 2));
                                //days[index].setPreferredSize(new Dimension(42,23));
		...
		if (weekOfYearVisible) {
			add(weekPanel, BorderLayout.WEST);
                ...
                shiftLess = new JButton("<<");
                shiftLess.setFocusPainted(false);
                shiftLess.setMargin(new Insets(0,0,0,0));
                
                shiftLess.addActionListener(new ActionListener(){

                @Override
                public void actionPerformed(ActionEvent e) {
                    firePropertyChange("shift",0,-1);
                }
                });
               
             
                shiftMore = new JButton(">>");
                shiftMore.setFocusPainted(false);
                shiftMore.setMargin(new Insets(0,0,0,0));
              
                shiftMore.addActionListener(new ActionListener(){
                ...
                shiftLess.setVisible(shiftLessVisible);
                
                topPanel.add(shiftLess);
                
                topPanel.add(jLabelTitle);
                
                shiftMore.setVisible(shiftMoreVisible);
                topPanel.add(shiftMore);
	...
	protected void drawDays() {
                if(calendar!=null)
                {
                    ...
                    jLabelTitle.setText( this.getMonthName(month) + " "+calendar.get(Calendar.YEAR));
                }
		Calendar tmpCalendar = (Calendar) calendar.clone();
		...
		while (day.before(firstDayInNextMonth)) {
			...
			while (iterator.hasNext()) {
				IDateEvaluator dateEvaluator = (IDateEvaluator) iterator.next();
                                //System.out.println("Checking if is special day "+day + " for de "+dateEvaluator);
                                
				if (dateEvaluator.isSpecial(day)) {
                                   // //System.out.println("it turns out to be special ");
	...
	public void setDay(int d) {
               ...
               if (d < 1) {
			//d = 1;
                    if(selectedDay!=null && selectedColor!=null)
                    {
...
//		}
		...
		for (int i = 7; i < 49; i++) {
			if (days[i].getText().equals(Integer.toString(day))) {
                                 ...
                                 if(selectedDay!=null)
                                 {
                                     //return the old selected day its original color
                                     selectedDay.setBackground(selectedColor);
                                     selectedDay.repaint();
                                 }
                            
                                 selectedDay = days[i];
                                 selectedColor = selectedDay.getBackground();
				 selectedDay.setBackground(selectedColor.darker());
                                 selectedDay.repaint();
				break;
			}
		}
...
//		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		int dayn = new Integer(buttonText).intValue();
		setDay(dayn);
	}
	...
	public static void main(String[] s) {
		...
		frame.getContentPane().add(new com.toedter.calendar.JDayChooser());
		frame.pack();
		frame.setVisible(true);
	}

    public boolean isShiftLessVisible() {
        return shiftLessVisible;
    }

    public void setShiftLessVisible(boolean shiftLessVisible) {
        this.shiftLessVisible = shiftLessVisible;
        this.shiftLess.setVisible(shiftLessVisible);
    }

    public boolean isShiftMoreVisible() {
        return shiftMoreVisible;
    }

    public void setShiftMoreVisible(boolean shiftMoreVisible) {
        this.shiftMoreVisible = shiftMoreVisible;
        this.shiftMore.setVisible(shiftMoreVisible);
    }
	...
	public void addDateEvaluator(IDateEvaluator dateEvaluator) {
                //System.out.println("added "+dateEvaluator);
    ...
    private String getMonthName(int m)
    {
        ...
        java.util.ResourceBundle bundle = java.util.ResourceBundle.getBundle("javar/javarbundle"); 
